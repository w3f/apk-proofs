...
impl<'a> Prover<'a> {
    ...
    pub fn prove(&self, bitmask: &Bitmask) -> Proof {
        ...
        let w1 = KZG_BW6::aggregate_polynomials(nu, &[self.session.pks_x_poly.clone(), self.session.pks_y_poly.clone(), b_poly, q_poly, acc_poly, c_poly, acc_x_poly, acc_y_poly]);
